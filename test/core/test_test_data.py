...
class JSONTestDataParserTest(TestCase):
    ...
    def test_empty(self):
        ...
        self.assertEqual([], json_parser(File(StringIO(''))))
        self.assertEqual([], json_parser(File(StringIO('{}'))))

    def test_basic(self):
        data = json_parser(File(StringIO(TEST_DATA)))
        self.assertEqual(6, len(data))

    def test_test_name(self):
        test_names = [t['test_name'] for t in json_parser(File(StringIO(TEST_DATA)))]
        self.assertIn("ungrouped_pass", test_names)
        self.assertIn("pass", test_names)

    def test_group_name(self):
        group_names = [t['group_name'] for t in json_parser(File(StringIO(TEST_DATA)))]
    ...
    def test_group_name_defaults_to_slash(self):
        data = json_parser(File(StringIO('{"/mytest0": "pass", "mytest1": "fail"}')))
